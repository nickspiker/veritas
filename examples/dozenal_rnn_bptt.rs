//! Dozenal RNN with Full BPTT - Sequential Encoding Test
//!
...
//!
//! Critical test: Does sequential encoding (vs bag-of-bytes) help network
//! learn positional structure across the ASCII gap?
//!
//! Hypothesis: RNN with sequential processing will learn that 'A' follows '9'
//! in the positional number system, despite ASCII discontinuity.
...
const LEARNING_RATE_DEN: u16 = 10000; // LR = 0.0001 (100x smaller)
...
fn main() {
    ...
    let lr_display = ScalarF4E4::from(LEARNING_RATE_NUM) / ScalarF4E4::from(LEARNING_RATE_DEN as u32);
    println!("Learning rate: {} (0.0001 - reduced 100x)", lr_display);
    println!();
    ...
    let lr = ScalarF4E4::from(LEARNING_RATE_NUM) / ScalarF4E4::from(LEARNING_RATE_DEN as u32);
    let mut optimizer = SGD::new(lr);

    println!("Training with full BPTT for {} epochs...\n", EPOCHS);
    ...
    for epoch in 0..EPOCHS {
        ...
        if epoch % 25 == 0 || epoch < 5 || epoch == EPOCHS - 1 {
    ...
    println!("\n=== Training Complete ===");
    println!("Test: Does sequential RNN learn positional structure?");
    println!("Comparison: Feedforward (bag-of-bytes) vs RNN (sequential)");
}
